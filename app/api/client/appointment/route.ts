...
//   Appointment fields: appointmentDate, appointmentTime, meetingMode?, purpose?, duration?
...
export async function POST(req: NextRequest) {
  ...
  // ── 3. Validate client fields via Zod ────────────────────────────────
  // Client password is submitted by the form.
  ...
  const clientParse = createClientSchema
    .omit({ password: true })   // passwordHash is server-generated
    .safeParse({
      name:        get("name"),
      email:       get("email"),
      phone:       get("phone"),
